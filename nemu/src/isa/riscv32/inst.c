...
static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2, word_t *imm, int *csr, int type) {
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, int tem = csrs(imm); csrs(imm) |= src1; R(rd) = tem; CSR++);                                                              
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, int tem = csrs(imm); csrs(imm)  = src1; R(rd) = tem; CSR++);                                                                
  ...
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , R, CSR++; s -> dnpc = csrs(MEPC));
